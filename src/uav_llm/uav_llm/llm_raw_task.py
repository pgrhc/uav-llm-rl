...
class LLMMissionPlanner:
    ...
    def _call_llm(self, user_command: str) -> str:
        prompt_text = f"{SYSTEM_PROMPT}\n\nCurrent Drone State: {self.current_state}\n\nCommand: \"{user_command}\"\nOutput:"
        payload = {
            "model": self.model,
            "prompt": prompt_text,
            "stream": False,
    ...
    def _extract_first_number_after(self, text: str, patterns: list[str], default_value: float) -> Optional[float]:
        for pat in patterns:
            ...
            if m:
                ...
                num_match = re.match(r"^(?:to|by|for|of)?\s*(-?\d+(\.\d+)?)", tail)
                if num_match:
    ...
    def _plan_from_llm(self, user_command: str) -> MissionPlan:
        ...
        for idx, step in enumerate(cmd.get("steps", []), start=1):
            ...
            if not self._validate_action(action):
                raise ValueError(f"Invalid action generated by LLM: {action}")
...
class LLMTaskNode(Node):
    def __init__(self):
        ...
        self.system_summary_sub = self.create_subscription(
            String,
            "/llm/system_summary",
            self.system_summary_callback,
            10,
        )
        self.latest_system_summary = ""
    ...
    def system_summary_callback(self, msg: String):
        self.latest_system_summary = msg.data
    ...
    def feedback_callback(self, msg: String):
        try:
            ...
            self.get_logger().error(f"Failed to process feedback: {e}")
