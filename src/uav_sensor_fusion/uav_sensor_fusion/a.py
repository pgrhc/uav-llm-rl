...
class ThreatEncoderNode(Node):
    def __init__(self):
        ...
        # --- Metrik takip değişkenleri ---
        self.tick_latencies_ms = deque(maxlen=100)   # Son 100 tick gecikmesi
        self.tracks_created_this_tick = 0
        self.tracks_deleted_this_tick = 0
        ...
        # --- Yeni metrik yayıncısı ---
        # /threat/metrics konusuna JSON formatında metrikler yayımlanır.
        # İçerik:
        #   tick_latency_ms   : Bu tick'in işlem süresi (ms)
        #   avg_latency_ms    : Son 100 tick ortalaması (ms)
        #   num_tracks        : Aktif iz sayısı
        #   max_threat_score  : Top-K içindeki en yüksek tehdit skoru
        #   tracks_created    : Bu tick'te başlatılan iz sayısı
        #   tracks_deleted    : Bu tick'te silinen iz sayısı
        #   yolo_active       : YOLO verisinin bu döngüde mevcut olup olmadığı
        #   radar_active      : Radar verisinin bu döngüde mevcut olup olmadığı
        #   lidar_active      : LiDAR verisinin bu döngüde mevcut olup olmadığı
        self.pub_metrics = self.create_publisher(String, "/threat/metrics", 10)
        ...
        self.get_logger().info(
            "ThreatEncoderNode V2 started (74-element state, 7-feature tokens, metrics enabled)")
    ...
    def _process_lidar(self, scan_msg: LaserScan):
        ...
        for i in range(self.lidar_sectors):
            ...
            end_idx = (start_idx + rays_per_sector
                       if i < self.lidar_sectors - 1 else num_rays)

            sector_rays = ranges[start_idx:end_idx]
            min_dist = float(np.min(sector_rays)) if len(sector_rays) > 0 else self.lidar_max_range
    ...
    def _update_tracks(self, matches, yolo_t: float, radar_t: float):
        ...
        for yd, rc in matches:
            ...
            if tid is None:
                ...
                self.tracks_created_this_tick += 1  # Yeni iz sayacı
        ...
        self.tracks_deleted_this_tick = len(to_del)  # Silinen iz sayacı
        for tid in to_del:
            del self.tracks[tid]
    ...
    def _publish_metrics(self, tick_latency_ms: float,
                         num_tracks: int,
                         ...
                         lidar_active: bool):
        """
        /threat/metrics konusuna JSON formatında metrik yayımlar.
        Her senaryo kaydı için ros2 topic echo /threat/metrics komutuyla
        okunabilir veya rosbag ile kaydedilebilir.
        """
        self.tick_latencies_ms.append(tick_latency_ms)
        avg_latency = float(np.mean(self.tick_latencies_ms))

        metrics = {
            "tick_latency_ms":  round(tick_latency_ms, 3),
            "avg_latency_ms":   round(avg_latency, 3),
            "num_tracks":       num_tracks,
            "max_threat_score": round(max_threat_score, 4),
            "tracks_created":   self.tracks_created_this_tick,
            "tracks_deleted":   self.tracks_deleted_this_tick,
            "yolo_active":      int(yolo_active),
            "radar_active":     int(radar_active),
            "lidar_active":     int(lidar_active),
        }
    ...
    def tick(self):
        # --- Tick başlangıç zamanı (gecikme ölçümü için) ---
        ...
        if radar_msg is None and yolo_msg is None and lidar_msg is None:
            ...
            tick_latency_ms = (time.perf_counter() - tick_start) * 1000.0
        ...
        if yolo_msg is not None:
            for d in yolo_msg.detections:
                ...
                yolo_list.append({
                    "pos": (float(hypo.pose.pose.position.x),
                            float(hypo.pose.pose.position.y)),
        ...
        # --- Maksimum tehdit skoru ---
        max_threat = float(topk[0][0]) if topk else 0.0
        ...
        for s, tid, token, tr in topk:
            ...
            parts.append(
                ...
                f"closing={closing:.2f} conf={conf:.2f} score={s:.4f}"
            )
        ...
        self._publish_metrics(
            ...
            yolo_active=yolo_fresh,
            radar_active=radar_fresh,
            lidar_active=lidar_fresh,
        )
